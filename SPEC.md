...
**Coverage (the librarian — BUILT, 2026-07-09):** when a scan hits a food the catalogue doesn't know, the app fires the `librarian` Edge Function after the meal persists. Per name it decides **alias-of-existing / new food / not-a-food** (dish names are refused — components log instead), generates a full per-serving profile — fiber composition **only from the existing fiber types**, `typical_serving_g`, colors / phytochemicals / guilds **only from the existing vocabularies**, coarse macro tiers, a `plants` row (with field-guide description) when it's a genuinely new plant — inserts it **`verified = false`, live instantly** (owner decision), links the item into the triggering meal (`source = 'librarian'`, keeping the camera's gram estimate), and logs a `librarian_added` ledger event. The result screen shows "New to us — adding it to the garden…" and the row heals in when the verdict lands; failures fall back to the manual "Pick a match." The owner reviews/amends the Studio queue (`foods`/`plants` where `verified = false`, rationale in `librarian_notes`) at leisure; edits propagate. Batch modes (service-role-only) backfilled fiber compositions for the whole catalogue and pre-seeded ~250 common ingredient-level names. The catalogue stays **ingredient-level** (bounded, reviewable); vision's decomposition handles dishes. Generation model: `claude-sonnet-4-6`; generated content is Fence 2/4 placeholder clinical data like the rest of the catalogue.
