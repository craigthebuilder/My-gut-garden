...
3. **The recognition pipeline contract** (`SPEC.md §4`) — the Edge Function and the versioned vision-LLM JSON output contract (**v2 as of 2026-07-08**: + `household_measure`/`est_grams` + component decomposition; v2 fields optional-on-decode). Stub the LLM call behind an interface so modules develop against fixtures. **Photos are stored permanently** (no expiry/cleanup) — per-user private, user-deletable.
...
2. **The LLM never produces composition numbers.** (Amended 2026-07-08, contract v2.) Vision estimates **what it can see** — food identity (mixed dishes decomposed to components) and **quantity** (a hand-anchored household measure + `est_grams`) — and nothing else. The **database** produces all fiber/phytochemical/guild/color/macro values, so the same food yields the same numbers every day. A per-scan LLM composition value is still a defect. (`§4`)
3. **Quantity is an estimate, never a measurement.** (Amended 2026-07-08.) Amounts surface as "~a fist · ~120g" — always "~"-softened, always user-adjustable (the ½×–2× review slider) — and `portion_tier` is retained as the coarse shadow of the grams ratio. Never surface grams as if weighed. Never "12.3g inulin." Directional, and say so. (`§4`, `§5`)
4. **Never diagnose — this is a wellness app.** The app never asserts a medical condition, and never asserts anything about the *user's own* microbiome (no "your Akkermansia is low"). Owner-authorized amendment (2026-08-13): naming a microbe in **educational** copy (e.g. a guild's function_copy describing what butyrate producers do) is allowed — the garden is explicitly a picture of what the user feeds, not a lab test. The four claim-implying guild *names* were de-claimed instead (see `FENCES.md` Fence 1). Allergy is **user-self-classified**; the strongest the app goes is a *"worth raising with a doctor/allergist"* care prompt. (`§9`, `§11`, `§15`)
